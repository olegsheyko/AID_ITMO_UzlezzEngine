...
int main(int argc, char** argv)
{
    ...
    TracySetProgramName("demo_messages");
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"demo_messages - Tracy / ImGui / D3D12", WS_OVERLAPPEDWINDOW, 100, 100, (int)(1000 * main_scale), (int)(700 * main_scale), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    // Демо «Сообщения»: литерал — в Tracy попадёт как есть, без копирования в рантайме
    TracyMessageL("level loaded");
    ...
    while (!done && !options.expired())
    {
        ...
        // Start the Dear ImGui frame
        {
            ...
            DemoPanel("demo_messages | Timeline logs", "4 / 5", "Open Messages. Filter by spawn, warning or checkpoint; select a message to jump to its time.");
            static int count = 5;
            static int total = 0;
            static int lastAutoTick = -1;
            ImGui::SliderInt("Entities per spawn", &count, 1, 100);
            bool spawn = ImGui::Button("Spawn");
            if (options.automatic && int(options.elapsed()) != lastAutoTick) {
                lastAutoTick = int(options.elapsed()); spawn = true;
            }
            if (spawn) {
                ZoneScopedNC("Spawn entities", 0x2ECC71);
                total += count;
                char text[96];
                const int size = std::snprintf(text, sizeof(text), "spawned %d entities; total=%d", count, total);
                TracyMessageC(text, size, 0x2ECC71);
                DemoBurn(50000);
            }
            ImGui::SameLine();
            if (ImGui::Button("Warning") || (spawn && options.automatic)) { TracyMessageLC("warning: demo budget exceeded", 0xE67E22); }
            ImGui::SameLine();
            if (ImGui::Button("Checkpoint") || (spawn && options.automatic)) { TracyMessageLC("checkpoint: state saved", 0x3498DB); }
            ImGui::Text("Spawned: %d", total);
            TracyPlot("Entities", (int64_t)total);
            ImGui::End();
        }
    ...
    std::printf("demo_messages: completed %u frames\n", g_frameIndex);
    return exitCode;
}
