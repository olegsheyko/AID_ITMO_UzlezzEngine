...
int main(int argc, char** argv)
{
    ...
    TracySetProgramName("demo_zones");
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"demo_zones - Tracy / ImGui / D3D12", WS_OVERLAPPEDWINDOW, 100, 100, (int)(1000 * main_scale), (int)(700 * main_scale), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    bool done = false;
    int exitCode = 0;
#if defined(TRACY_ENABLE) && defined(TRACY_SAMPLING_PROFILER_MANUAL_START)
    ...
    while (!done && !options.expired())
    {
        ...
        // Start the Dear ImGui frame
        {
            ...
            DemoPanel("demo_zones | CPU zones and frames", "1 / 5", "Expand Main > Frame > GameLogic. Compare colored nested zones and frame spikes.");
            static int work = 80000;
            static bool spikes = true;
            ImGui::SliderInt("CPU iterations", &work, 1000, 400000);
            ImGui::Checkbox("Spike every 120 frames", &spikes);
            ImGui::TextUnformatted("Blue: Update; orange: Physics; purple: AI; red: occasional spike.");
            ImGui::TextUnformatted("GameLogic has text + frame value. Asset batch has a runtime name.");
#ifdef TRACY_DEMO_DETAIL
            ImGui::TextUnformatted("Detail build: Update also captures an 8-frame call stack.");
#endif
#if defined(TRACY_ENABLE) && defined(TRACY_SAMPLING_PROFILER_MANUAL_START)
            if (!samplingRequested && ImGui::Button("Start sampling"))
                samplingRequested = tracy::BeginSamplingProfiling();
            if (samplingRequested && ImGui::Button("Stop sampling")) {
                tracy::EndSamplingProfiling();
                samplingRequested = false;
            }
            ImGui::TextWrapped("Sampling needs OS permissions; verify collected samples in Tracy Statistics.");
#endif
            ImGui::End();
            FrameMarkStart("Simulation");
            {
                ZoneScopedNC("GameLogic", 0xDC3545);
                ZoneText("Whole workload, not individual iterations", sizeof("Whole workload, not individual iterations") - 1);
                {
#ifdef TRACY_DEMO_DETAIL
                    ZoneScopedNCS("Update", 0x3498DB, 8);
#else
                    ZoneScopedNC("Update", 0x3498DB);
#endif
                    DemoBurn(work);
                }
                { ZoneScopedNC("Physics", 0xE67E22); DemoBurn(work / 2); }
                { ZoneScopedNC("AI", 0x9B59B6); DemoBurn(work / 3); }
                {
                    char name[64];
                    std::snprintf(name, sizeof(name), "Asset batch %u", g_frameIndex % 3);
                    ZoneTransientN(batchZone, name, true);
                    ZoneTextV(batchZone, "Dynamic name, same-thread scope", sizeof("Dynamic name, same-thread scope") - 1);
                    DemoBurn(work / 4);
                }
                if (spikes && g_frameIndex % 120 == 0) {
                    ZoneScopedNC("Frame spike", 0xE74C3C);
                    DemoBurn(work * 12);
                }
                ZoneValue(g_frameIndex);
            }
            FrameMarkEnd("Simulation");
        }
...
#if defined(TRACY_ENABLE) && defined(TRACY_SAMPLING_PROFILER_MANUAL_START)
    if (samplingRequested) tracy::EndSamplingProfiling();
#endif
    ...
    std::printf("demo_zones: completed %u frames\n", g_frameIndex);
    return exitCode;
}
