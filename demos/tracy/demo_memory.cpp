...
// Explicit named pools avoid globally replacing new/delete (including profiler allocations).
static constexpr int MEM_CALLSTACK_DEPTH = 10;
...
int main(int argc, char** argv)
{
    ...
    TracySetProgramName("demo_memory");
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"demo_memory - Tracy / ImGui / D3D12", WS_OVERLAPPEDWINDOW, 100, 100, (int)(1000 * main_scale), (int)(700 * main_scale), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    // Route ImGui's internal allocations (it uses malloc by default) through
    // a named Tracy pool "ImGui", so they show up as a separate category
    ImGui::SetAllocatorFunctions(
        [](size_t sz, void*) -> void*
        {
            void* p = std::malloc(sz);
            if (p) TracyAllocNS(p, sz, MEM_CALLSTACK_DEPTH, "ImGui");
            return p;
        },
        [](void* p, void*)
        {
            if (!p) return;
            if (p) TracyFreeNS(p, MEM_CALLSTACK_DEPTH, "ImGui");
            std::free(p);
        },
        nullptr);
    ...
    // Memory demo state
    std::vector<char*> mem_leaked;          // "утёкшие" блоки (освобождаются по кнопке)
    int  mem_allocs_per_frame = 64;         // мелких аллокаций в горячем пути за кадр
    bool mem_simulate_leak = false;         // включить имитацию утечки
    ...
    while (!done && !options.expired())
    {
        ...
        // Start the Dear ImGui frame
        {
            ...
            DemoPanel("demo_memory | Allocations", "2 / 5", "Open Memory. Compare named pools HotPath, Leaky and ImGui; inspect allocation stacks.");
            ImGui::SliderInt("Allocations per frame", &mem_allocs_per_frame, 0, 512);
            ImGui::Checkbox("Retain 64 KB per frame (max 32 MB)", &mem_simulate_leak);
            bool freeBlocks = ImGui::Button("Free retained blocks");
            if (options.automatic) {
                mem_simulate_leak = std::fmod(options.elapsed(), 4.0) < 2.0;
                freeBlocks |= !mem_simulate_leak && !mem_leaked.empty();
            }
            if (freeBlocks) {
                for (char* p : mem_leaked) { TracyFreeNS(p, MEM_CALLSTACK_DEPTH, "Leaky"); std::free(p); }
                mem_leaked.clear();
            }
            ImGui::Text("Retained: %zu blocks / %.2f MB", mem_leaked.size(), mem_leaked.size() * 4096.0 / (1024 * 1024));
            ImGui::TextUnformatted("The retained blocks model a leak; Free and exit release them.");
            ImGui::End();
        }
        ...
        // Simulated memory workload for profiling demo
        {
            ZoneScopedNC("MemoryWorkload", tracy::Color::Orange);

            {
                ZoneScopedN("CppNewAllocs");
                for (int i = 0; i < mem_allocs_per_frame; ++i) {
                    // Explicit calls keep the teaching workload observable in optimized builds.
                    auto* p = static_cast<volatile char*>(::operator new[](256));
                    p[0] = static_cast<char>(i);
                    ::operator delete[](const_cast<char*>(p));
                }
            }

            // Горячий путь: много мелких аллокаций каждый кадр.
            // Именованный пул "HotPath" — в Tracy аллокации группируются по именам.
            {
                ZoneScopedN("HotPathAllocs");
                for (int i = 0; i < mem_allocs_per_frame; ++i)
                {
                    char* p = (char*)std::malloc(256);
                    if (!p) break;
                    TracyAllocNS(p, 256, MEM_CALLSTACK_DEPTH, "HotPath");
                    p[0] = (char)i;
                    TracyFreeNS(p, MEM_CALLSTACK_DEPTH, "HotPath");
                    std::free(p);
                }
            }

            // Утечка: аллокации без пары free — график памяти ползёт вверх.
            // Отдельный именованный пул "Leaky", чтобы отличать их от HotPath.
            if (mem_simulate_leak && mem_leaked.size() < 8192)
            {
                ZoneScopedN("LeakyAllocs");
                for (int i = 0; i < 16; ++i)
                {
                    char* p = (char*)std::malloc(4096);
                    if (!p) break;
                    TracyAllocNS(p, 4096, MEM_CALLSTACK_DEPTH, "Leaky");
                    mem_leaked.push_back(p);
                }
            }

            TracyPlot("Leaked bytes", (double)mem_leaked.size() * 4096.0);
            TracyPlot("Allocs per frame", (double)mem_allocs_per_frame);
        }
    ...
    // Освобождаем накопленные демо-блоки, чтобы не тащить утечку в выход
    for (char* p : mem_leaked)
    {
        TracyFreeNS(p, MEM_CALLSTACK_DEPTH, "Leaky");
        std::free(p);
    }
    mem_leaked.clear();
    ...
    std::printf("demo_memory: completed %u frames\n", g_frameIndex);
    return exitCode;
}
